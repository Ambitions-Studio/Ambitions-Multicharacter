...
<template>
  <div v-if="isVisible || props.forceVisible" class="fixed inset-0 w-full h-full z-10" style="background: linear-gradient(to right, rgba(15, 23, 42, 1) 0%, rgba(15, 23, 42, 0.7) 15%, rgba(15, 23, 42, 0.4) 33%, transparent 100%)">
    ...
    <Transition
      ...
      appear
    >
      <div class="absolute left-16 top-16 w-[32rem] fhd:w-[28rem] 2k:w-[40rem] max-h-[85vh] overflow-hidden">
      ...
      <div class="mb-8">
        ...
        <h1 class="text-4xl fhd:text-3xl 2k:text-6xl font-black text-white leading-tight tracking-wide">
          Character <span class="text-3xl fhd:text-2xl 2k:text-5xl font-light text-blue-200/80">Creator</span>
        </h1>
      </div>
      ...
      <div v-if="currentStep === 0" class="space-y-6">
        <div class="flex items-center space-x-3 mb-6">
          ...
          <span class="text-slate-300 text-sm font-medium uppercase tracking-widest">Ped Model Selection</span>
        </div>
        ...
        <div class="bg-slate-800/50 rounded-xl p-6 border border-slate-600/30">
          ...
          <div v-if="selectedPed" class="mt-4 p-4 bg-slate-700/50 rounded-lg border border-blue-500/20">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-white font-medium">Selected Model</p>
                <p class="text-slate-300 text-sm">{{ pedModels.find(p => p.value === selectedPed)?.title }}</p>
              </div>
              <div class="text-right">
                <p class="text-slate-400 text-xs">Category</p>
                <p class="text-blue-300 text-sm font-medium">{{ pedModels.find(p => p.value === selectedPed)?.category }}</p>
              </div>
            </div>
          </div>
        </div>
